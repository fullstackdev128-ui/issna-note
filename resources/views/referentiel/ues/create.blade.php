...
@section('title', 'Nouvelle Unité d\'Enseignement (UE)')
...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-black uppercase tracking-tighter text-gray-800">Nouvelle UE</h1>
    ...
    <form action="{{ route('referentiel.ues.store') }}" method="POST" class="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 space-y-6">
        @csrf
        ...
        <div class="space-y-4">
            <div>
                ...
                <select name="specialite_id" class="w-full bg-gray-50 border-none rounded-xl px-4 py-3 font-bold text-gray-700 focus:ring-2 focus:ring-blue-500 transition-all">
                    ...
                    @foreach($specialites as $s)
                        <option value="{{ $s->id }}" {{ old('specialite_id') == $s->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="code_ue" value="{{ old('code_ue') }}" placeholder="Ex: SIN111" class="w-full bg-gray-50 border-none rounded-xl px-4 py-3 font-bold text-gray-700 focus:ring-2 focus:ring-blue-500 transition-all">
                </div>
                <div>
                    ...
                    <select name="type_ue" class="w-full bg-gray-50 border-none rounded-xl px-4 py-3 font-bold text-gray-700 focus:ring-2 focus:ring-blue-500 transition-all">
                        <option value="Fondamentale" {{ old('type_ue') == 'Fondamentale' ? 'selected' : '' }}>Fondamentale</option>
                        <option value="Professionnelle" {{ old('type_ue') == 'Professionnelle' ? 'selected' : '' }}>Professionnelle</option>
                        <option value="Transversale" {{ old('type_ue') == 'Transversale' ? 'selected' : '' }}>Transversale</option>
                    </select>
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" name="nom" value="{{ old('nom') }}" placeholder="Ex: Sciences Biologiques" class="w-full bg-gray-50 border-none rounded-xl px-4 py-3 font-bold text-gray-700 focus:ring-2 focus:ring-blue-500 transition-all">
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Niveau</label>
                    <input type="number" name="niveau" value="{{ old('niveau', 1) }}" min="1" max="5" class="w-full bg-gray-50 border-none rounded-xl px-4 py-3 font-bold text-gray-700 focus:ring-2 focus:ring-blue-500 transition-all">
                </div>
                <div>
                    <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Semestre</label>
                    <input type="number" name="semestre" value="{{ old('semestre', 1) }}" min="1" max="2" class="w-full bg-gray-50 border-none rounded-xl px-4 py-3 font-bold text-gray-700 focus:ring-2 focus:ring-blue-500 transition-all">
                </div>
            </div>
        </div>
