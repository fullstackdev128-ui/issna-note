...
@section('title', 'Nouvelle Matière (EC)')

@section('content')
<div class="max-w-2xl mx-auto space-y-6">
    <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
        <h1 class="text-2xl font-black uppercase tracking-tighter text-gray-800">Ajouter une Matière</h1>
        <p class="text-sm text-gray-400 font-bold uppercase tracking-widest">Référentiel Académique</p>
    </div>

    <form action="{{ route('referentiel.ecs.store') }}" method="POST" class="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 space-y-6">
        @csrf
        ...
        <div class="space-y-4">
            <div>
                ...
                <select name="ue_id" class="w-full bg-gray-50 border-none rounded-xl px-4 py-3 font-bold text-gray-700 focus:ring-2 focus:ring-blue-500 transition-all">
                    @foreach($ues as $ue)
                        <option value="{{ $ue->id }}" {{ old('ue_id', $ueId) == $ue->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="code_ec" value="{{ old('code_ec') }}" placeholder="Ex: BIO111" class="w-full bg-gray-50 border-none rounded-xl px-4 py-3 font-bold text-gray-700 focus:ring-2 focus:ring-blue-500 transition-all">
                ...
                <div>
                    ...
                    <input type="number" name="credit" value="{{ old('credit', 2) }}" min="1" max="10" class="w-full bg-gray-50 border-none rounded-xl px-4 py-3 font-bold text-gray-700 focus:ring-2 focus:ring-blue-500 transition-all">
            ...
            <div>
                ...
                <input type="text" name="nom" value="{{ old('nom') }}" placeholder="Ex: Anatomie et Physiologie" class="w-full bg-gray-50 border-none rounded-xl px-4 py-3 font-bold text-gray-700 focus:ring-2 focus:ring-blue-500 transition-all">
            ...
            <div>
                ...
                <input type="number" step="0.25" name="note_eliminatoire" value="{{ old('note_eliminatoire') }}" placeholder="Ex: 07.00" class="w-full bg-gray-50 border-none rounded-xl px-4 py-3 font-bold text-gray-700 focus:ring-2 focus:ring-blue-500 transition-all">
        ...
        <div class="flex pt-4 space-x-4">
            ...
            <a href="{{ route('referentiel.ecs.index', ['ue_id' => $ueId]) }}" class="px-6 py-4 rounded-2xl font-black uppercase tracking-widest text-xs text-gray-400 hover:bg-gray-50 transition-all text-center">
                Annuler
            </a>
        </div>
    </form>
