...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
        <h1 class="text-2xl font-black uppercase tracking-tighter text-gray-800">Ajouter une Année</h1>
        <p class="text-sm text-gray-400 font-bold uppercase tracking-widest">Référentiel Académique</p>
    </div>

    <form action="{{ route('referentiel.annees.store') }}" method="POST" class="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 space-y-6">
        @csrf
        
        <div class="space-y-4">
            <div>
                <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Libellé</label>
                <input type="text" name="libelle" value="{{ old('libelle') }}" placeholder="Ex: 2025-2026" class="w-full bg-gray-50 border-none rounded-xl px-4 py-3 font-bold text-gray-700 focus:ring-2 focus:ring-blue-500 transition-all">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="date_debut" value="{{ old('date_debut') }}" class="w-full bg-gray-50 border-none rounded-xl px-4 py-3 font-bold text-gray-700 focus:ring-2 focus:ring-blue-500 transition-all">
                ...
                <div>
                    ...
                    <input type="date" name="date_fin" value="{{ old('date_fin') }}" class="w-full bg-gray-50 border-none rounded-xl px-4 py-3 font-bold text-gray-700 focus:ring-2 focus:ring-blue-500 transition-all">
