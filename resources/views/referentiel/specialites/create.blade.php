...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="bg-[#1e3a8a] px-8 py-6 text-white">
            <h3 class="text-2xl font-black uppercase tracking-tighter">Nouvelle Spécialité</h3>
            <p class="text-blue-200 text-sm mt-1">Créez un nouveau parcours de formation.</p>
        </div>

        <form action="{{ route('referentiel.specialites.store') }}" method="POST" class="p-8 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" name="duree_ans" value="{{ old('duree_ans', 3) }}" min="1" max="10" required 
            ...
            <div class="space-y-2">
                ...
                <select name="type_diplome" required 
                    class="w-full px-5 py-4 bg-gray-50 border-2 border-gray-100 rounded-2xl focus:border-blue-600 outline-none font-bold text-gray-800 transition-all">
                    <option value="BTS" {{ old('type_diplome') == 'BTS' ? 'selected' : '' }}>BTS</option>
                    <option value="Licence" {{ old('type_diplome') == 'Licence' ? 'selected' : '' }}>Licence</option>
                    <option value="Master1" {{ old('type_diplome') == 'Master1' ? 'selected' : '' }}>Master 1</option>
                    <option value="Master2" {{ old('type_diplome') == 'Master2' ? 'selected' : '' }}>Master 2</option>
                </select>
            ...
            <div class="flex items-center space-x-4 p-4 bg-gray-50 rounded-2xl">
                ...
                <input type="checkbox" name="actif" value="1" id="actif" {{ old('actif', '1') == '1' ? 'checked' : '' }}
